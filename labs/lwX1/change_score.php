<?php
    ...
    require_once(SCRIPTS_DIR . 'change_score.inc.php');
    ...
    $isCorrectRequest = !is_null($newChange['file_id']) &&
                        !is_null($newChange['user_id']) &&
                        !is_null($isPlus) &&
                        checkUserChangeFile($newChange['user_id'], $newChange['file_id']);

    if ($isCorrectRequest)
    {
        $where = 'file_id=' . $newChange['file_id'];
        $table = 'user_files';
        $set = ($isPlus == 'true') ? 'file_score=file_score+1' : 'file_score=file_score+1';
