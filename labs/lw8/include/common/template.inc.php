<?php
    ...
    function buildPage($tplName, $tplVars, $pageVars)
    {
        return getView(TOP_PATH, $pageVars) . getView($tplName, $tplVars) . getView(BOT_PATH, $pageVars);
    }


    function getView2($templateName, $vars = array())
    {
        $content = '';
        $templatePath = TPL_PATH . $templateName;
        if (file_exists($templatePath))
        {
        ...
        if (!empty($vars))
        {
            ...
            $content = preg_replace('/[{][$]([a-zA-Z]{0,30})[}]/', '', $content);
        }
        return $content;
    }
